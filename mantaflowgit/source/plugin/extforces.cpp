...
inline void convertDescToVec(const std::string& desc, Vector3D<bool>& lo, Vector3D<bool>& up) {
...
PYTHON() void setOpenBound(FlagGrid& flags, int bWidth, std::string openBound = "", int type = FlagGrid::TypeOutflow | FlagGrid::TypeEmpty){
	if (openBound == "") return;
	Vector3D<bool> lo, up;
...
//! enforce a constant inflow/outflow at the grid boundaries
PYTHON() void setInflowBcs(MACGrid& vel, std::string dir, Vec3 value) {
