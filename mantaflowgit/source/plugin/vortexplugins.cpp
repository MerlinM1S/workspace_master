...
#if GOOGLE_CUDA == 0
        std::cout << "vorticity: max " << maxV << " / mean " << meanV/mesh.numTris() << std::endl;
#endif
}
...
PYTHON() void smoothVorticity(VortexSheetMesh& mesh, int iter=1, Real sigma=0.2, Real alpha=0.8)
{
	...
	// pre-calculate positions and weights
        std::vector<Vec3> vort(mesh.numTris()), pos(mesh.numTris());
        std::vector<Real> weights(3*mesh.numTris());
        std::vector<int> index(3*mesh.numTris());
