...
template<class T>
void Grid<T>::load(std::string name) {
...
template<class T>
void Grid<T>::save(std::string name) {
        if (name.find_last_of('.') == std::string::npos)
		errMsg("file '" + name + "' does not have an extension");
        std::string ext = name.substr(name.find_last_of('.'));
	if (ext == ".raw")
...
template<class T> void Grid<T>::printGrid(int zSlice, bool printIndex, int bnd) {
	...
	FOR_IJK_BND(*this,bnd) {
        ...
        out << std::endl; debMsg("Printing " << this->getName() << out.str().c_str() , 1);
}
...
void FlagGrid::initDomain( const int &boundaryWidth
                             , const std::string &wallIn
                                                 , const std::string &openIn
                                                 , const std::string &inflowIn
                                                 , const std::string &outflowIn
						 , Grid<Real>* phiWalls ) {
	
	int  types[6] = {0};
	bool set  [6] = {false};
	// make sure we have at least 6 entries
        std::string wall    = wallIn;    wall.append("      ");
        std::string open    = openIn;    open.append("      ");
        std::string inflow  = inflowIn;  inflow.append("      ");
        std::string outflow = outflowIn; outflow.append("      ");
