...
KERNEL(bnd=1)
void SemiLagrangeMAC(const FlagGrid& flags, const MACGrid& vel, MACGrid& dst, MACGrid& src, Real dt, int orderSpace)
{
...
void MacCormackCorrect(const FlagGrid& flags, Grid<T>& dst, Grid<T>& old, Grid<T>& fwd,  Grid<T>& bwd,
...
void MacCormackCorrectMAC(const FlagGrid& flags, Grid<T>& dst, Grid<T>& old, Grid<T>& fwd,  Grid<T>& bwd,
					   Real strength, bool isLevelSet, bool isMAC=false )
{
...
template<class T>
inline T doClampComponent(const Vec3i& gridSize, const FlagGrid& flags, T dst, Grid<T>& orig, T fwd, const Vec3& pos, const Vec3& vel, const int clampMode )
{
...
template<int c> 
inline Real doClampComponentMAC(const FlagGrid& flags, const Vec3i& gridSize, Real dst, MACGrid& orig, Real fwd, const Vec3& pos, const Vec3& vel, const int clampMode )
{
...
KERNEL(bnd=1) template<class T>
void MacCormackClamp(const FlagGrid& flags, const MACGrid& vel, Grid<T>& dst, Grid<T>& orig, Grid<T>& fwd, Real dt, const int clampMode)
{
	T     dval       = dst(i,j,k);
...
KERNEL(bnd=1) 
void MacCormackClampMAC (const FlagGrid& flags, const MACGrid& vel, MACGrid& dst, MACGrid& orig, MACGrid& fwd, Real dt, const int clampMode)
{
	Vec3  pos(i,j,k);
	Vec3  dval       = dst(i,j,k);
	Vec3  dfwd       = fwd(i,j,k);
...
KERNEL() void extrapolateVelConvectiveBC(const FlagGrid& flags, MACGrid& vel, MACGrid& velDst, MACGrid& velPrev, Real timeStep, int bWidth){
...
KERNEL() void copyChangedVels(const FlagGrid& flags, MACGrid& velDst, MACGrid& vel){ if (flags.isOutflow(i,j,k)) vel(i, j, k) = velDst(i, j, k); }

//! extrapolate normal velocity components into open boundary cells (marked as outflow cells)
void applyOutflowBC(const FlagGrid& flags, MACGrid& vel, MACGrid& velPrev, double timeStep, int bWidth=1) {
