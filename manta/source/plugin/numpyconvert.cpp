	...
	switch (eDataType)
	{
		case NumpyTypes::N_FLOAT:
			FOR_IJK(_Target) { for(int w = 0; w < 3; ++w) {
                                _Target(i,j,k).value[w] = reinterpret_cast<float*>(_Source.pData)[w + uSizeW * (k +  uSizeZ * (j + uSizeY * i))]; } }
			break;
		case NumpyTypes::N_DOUBLE:
			FOR_IJK(_Target) { for(int w = 0; w < 3; ++w) {
                                _Target(i,j,k).value[w] = reinterpret_cast<double*>(_Source.pData)[w + uSizeW * (k +  uSizeZ * (j + uSizeY * i))]; } }
			break;
		default:
			break;
	}
	...
	switch (eDataType)
	{
		case NumpyTypes::N_FLOAT:
			FOR_IJK(_Source) { for(int w = 0; w < 3; ++w) {
                                reinterpret_cast<float*>(_Target.pData)[w + uSizeW * (k +  uSizeZ * (j + uSizeY * i))] = _Source(i,j,k).value[w]; } }
			break;
		case NumpyTypes::N_DOUBLE:
			FOR_IJK(_Source) { for(int w = 0; w < 3; ++w) {
                                reinterpret_cast<double*>(_Target.pData)[w + uSizeW * (k +  uSizeZ * (j + uSizeY * i))] = _Source(i,j,k).value[w]; } }
			break;
		default:
			break;
	}
